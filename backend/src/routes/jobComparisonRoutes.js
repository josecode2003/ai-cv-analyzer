...
} = require('../services/jobComparisonService')
...
function formatComparison(comparison) {
  ...
  const cvAnalysisId =
    comparison.cv_analysis_id ??
    comparison.cvAnalysisId
  ...
  const jobOfferText =
    comparison.job_offer_text ??
    comparison.jobOfferText
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        status: 'error',
        message:
          'La oferta de empleo es obligatoria'
      })

    }


    const normalizedJobOffer =
      jobOfferText.trim()


    if (
      normalizedJobOffer.length < 50
    ) {
    ...
    if (
      normalizedJobOffer.length > 30000
    ) {
    ...
    if (
      normalizedJobTitle.length > 255
    ) {

      return res.status(400).json({
        status: 'error',
        message:
          'El título de la oferta no puede superar los 255 caracteres'
      })
...
if (existingComparison) {
  ...
  return res.status(200).json({
    ...
    message:
      'Esta comparación ya había sido realizada anteriormente',

  comparison:
      formatComparison(existingComparison),
...
return res.status(201).json({
  ...
  cached: false
      ...
      console.error(
        'Error comparando CV con oferta:',
        error
      )
      ...
      return res.status(500).json({
        ...
        message:
          'No se pudo comparar el CV con la oferta de empleo'
...
return res.status(200).json({
  ...
  comparisons:
    comparisons.map(
      ...
      console.error(
        'Error obteniendo comparaciones:',
        error
      )
      ...
      return res.status(500).json({
        ...
        message:
          'No se pudieron obtener las comparaciones'
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        ...
        message:
          'El ID de la comparación no es válido'
  ...
  comparison:
    formatComparison(comparison)
      ...
      return res.status(500).json({
        ...
        message:
          'No se pudo obtener la comparación'
